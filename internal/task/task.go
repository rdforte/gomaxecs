...
// GetMaxThreads is responsible for getting the max number of threads based on
// the CPU limit of the container and the task.
...
func GetMaxThreads(ecsMetaURI, containerID string) (int, error) {
	if len(ecsMetaURI) == 0 {
		return 0, fmt.Errorf("no container URI provided")
	}

	if len(containerID) == 0 {
		return 0, fmt.Errorf("no container ID provided")
	}
